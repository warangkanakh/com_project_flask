...
def timed_job():
    ...
    for i in searched_tweets:#loop for append to list and crate to CSV
        if 'RT @' not in i.full_text:
            ...
            count_searched += 1
            f=open('test_search.csv', 'a+',encoding="utf-8", newline='')
            writer = csv.writer(f)
            if(os.stat("test_search.csv").st_size == 0):
                writer.writerow(['Time','ID','Name','Text'])
            writer.writerow([i.created_at,i.author.id,i.author.screen_name,i.full_text])
            print(i.full_text)
            f.close()
